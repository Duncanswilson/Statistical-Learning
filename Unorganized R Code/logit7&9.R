...
##Seperate the 7's and 9's of training data  
SevensandNines <- (which(X.train[,1] == 7 | X.train[,1] == 9))
Y.train <- as.matrix(X.train[SevensandNines, 1] - 7 )  
Y.train <- (X.train[SevensandNines, 1]-7) 
...
##seperate the 7's and 9's in the test data 
...
for(i in 1:nrow(X.testSubset)){
  if(  (exp(t(beta2)%*%X.testSubset[i,])/(1+exp(t(beta2)%*%X.testSubset[i,]))) > (1/(1+exp(t(beta2)%*%X.testSubset[i,])))){
    X.BIprediction[i] = 1
    if(Y.test[i] == 1){ 
      BINineCorrect = BINineCorrect + 1  
    }
  }
  else{
    X.BIprediction[i] = 0 
    if(Y.test[i] == 0){
      BISevenCorrect = BISevenCorrect + 1
    }
  }
}
